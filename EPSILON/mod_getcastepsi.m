function [epsiup,epsidown,dataup,datadown] = mod_getcastepsi(data,CTDup,CTDdown)
...
epsidown=cellfun( @(x) find(data.epsitime>=x.time(1) & data.epsitime<=x.time(end)),CTDdown,'un',0);
epsiup=cellfun( @(x) find(data.epsitime>=x.time(1) & data.epsitime<=x.time(end)),CTDup,'un',0);
