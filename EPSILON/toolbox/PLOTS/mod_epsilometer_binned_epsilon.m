function mod_epsilometer_binned_epsilon(Meta_Data,MS,epsi_bin)
...
for i=1:length(epsi_bin)
    index1=cellfun(@(x) find(x.epsilon(:,1)>=.5*epsi_bin(i) & x.epsilon(:,1)<1.5*epsi_bin(i)),MS,'un',0);
    index2=cellfun(@(x) find(x.epsilon(:,2)>=.5*epsi_bin(i) & x.epsilon(:,2)<1.5*epsi_bin(i)),MS,'un',0);
    notempty_index1=find(~cellfun(@isempty,index1));
    notempty_index2=find(~cellfun(@isempty,index2));
    count_s1=1;
    count_s2=1;
    Psk1=0*k;
    kvis1=0;
