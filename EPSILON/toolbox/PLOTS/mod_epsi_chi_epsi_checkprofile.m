...
%     a3f_0=smoothdata(a3_0,'movmean',nsmooth);
    ...
    else;Cos1a2=0.*squeeze(MS{l}.Pf(1,:,:));end
...
for k=1:2:length(MS{l}.kvis)
    ...
    % coherence plot
    a=8;
    H=semilogx(ax(a),MS{l}.f,Cos1a1(k,:),'b');
    hold(ax(a),'on')
...
%     O=semilogx(ax(a),MS{l}.f,Cos1a3_0(k,:),'g');
    ...
    %acceleration plot
    a=7;
    K=loglog(ax(a),MS{l}.f,a1f(k,:),'b');
    hold(ax(a),'on')
    L=loglog(ax(a),MS{l}.f,a2f(k,:),'r');
    M=loglog(ax(a),MS{l}.f,a3f(k,:),'k');
    N=loglog(ax(a),MS{l}.f,s1f(k,:),'m');
    set(ax(a),'Xticklabel','')
    grid(ax(a),'on')
