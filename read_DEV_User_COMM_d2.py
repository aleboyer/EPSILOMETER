...
if len(sys.argv)>=2:
   filename=sys.argv[1]
   fid,eof=open_datafile('/Volumes/KINGSTON/DEV/User_COMM/d2/raw/'+filename)
   epsifile='/Volumes/KINGSTON/DEV/User_COMM/d2/epsi/'+filename
else:
   filename='/Volumes/KINGSTON/DEV/User_COMM/d2/raw/MADRE_CALFPO7SBE.dat' 
   fid,eof=open_datafile(filename)
   epsifile='/Volumes/KINGSTON/DEV/User_COMM/d2/epsi/MADRE_CALFPO7SBE.dat'
   
if len(sys.argv)==3:
    ctdfile= '/Volumes/KINGSTON/DEV/User_COMM/d2/ctd/'+sys.argv[2]
else:
    ctdfile='/Volumes/KINGSTON/DEV/User_COMM/d2/ctd/MADRE_CALFPO7SBE.dat'
...
for i,block in enumerate(blocks[1:-1]):
    ...
    for j,dev_block in enumerate(Splitblock):
        if j==0:
            EpsiStamp[i]     = int(dev_block.split(b',')[0],10)
            TimeStamp[i]     = int(dev_block.split(b',')[1],10)
