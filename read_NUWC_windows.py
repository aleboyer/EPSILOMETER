...
from EPSIlib import EPSI_channel_univolt
            ...
            if j==0:
                ...
                if dev_block[:4]==b'EPSI':
                    ...
                    for k,sample in enumerate(epsiblock):
                            t2[i*len(epsiblock)+k]=EPSI_channel_univolt(sample,0)
                            s1[i*len(epsiblock)+k]=EPSI_channel_univolt(sample,6)
                            s2[i*len(epsiblock)+k]=EPSI_channel_univolt(sample,12)
                            a1[i*len(epsiblock)+k]=EPSI_channel_big(sample,18)
                            a3[i*len(epsiblock)+k]=EPSI_channel_big(sample,24)
    except:
