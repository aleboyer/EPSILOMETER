...
## DO NOT TOUCH 
def open_datafile_write(filename='/Volumes/KINGSTON/DEV/User_COMM/d2/raw/MADRE_CALFPO7SBE.dat'):
...
if len(sys.argv)==2:
   filename=sys.argv[1]
   fid=open_datafile_write('/Volumes/KINGSTON/DEV/User_COMM/d2/raw/' +filename)
else:
...
#          print('wait')
          ...
          if line[:6]==b'$MADRE':
             #State=1
             len_header    = len(line)
             EpsiStamp     = int(line[6:6+10],10)
             TimeStamp     = int(line[17:17+10],10)
             Voltage       = int(line[28:28+10],16)
             Checksum_aux1 = int(line[39:39+10],16)
             Checksum_aux2 = int(line[50:50+10],16)
             Checksum_map  = int(line[61:61+10],16)
