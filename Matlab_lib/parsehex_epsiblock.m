function [index,timestamp,sderror,chsum1,alti,chsumepsi,...
          ...
          EPSIchannels]=parsehex_epsiblock(Madreblock,Meta_Data)
...
Lepsisample = nb_channels*adcword+2;
...
if ~isempty(Meta_Data.SBEcal)
    AUX1raw   = Madreblock(ind_AUX1+7:ind_EPSI-3);
end
EPSIraw   = Madreblock(ind_EPSI+7:end);
...
if flagbadblock==0
    EPSIblock=arrayfun(@(x) EPSIraw(x:x+Lepsisample-3), ...
...
channel_count=@(x) (cell2mat(arrayfun(@(y)  ...
    (hex2dec(x(y:y+adcword-1))),...
    1:adcword:Lepsisample-2,'un',0)));
%
...
EPSIchannels=EPSI_volt2g(EPSIchannels,Meta_Data);
    ...
    EPSIchannels=zeros(Nepsisample,nb_channels);
end
