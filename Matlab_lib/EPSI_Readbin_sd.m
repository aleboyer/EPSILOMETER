function [EPSI,AUX,posi]=EPSI_Readbin_sd(file,posi)
    ...
    AUX.AUX1index=zeros(N_goodMadreblock);
    AUX.AUX1sample=char(zeros(9*N_goodMadreblock,22));
    ...
    for i=1:N_goodMadreblock
        ...
        [EPSI.index(i),EPSI.timestamp(i),EPSI.sderror(i),EPSI.chsum1(i),EPSI.alti(i),EPSI.chsumepsi(i),...
            AUX.AUX1index(1+(i-1)*9:i*9),AUX.AUX1sample(1+(i-1)*9:i*9,:),...
            EPSI.EPSIchannels(1+(i-1)*160:i*160,:)]=parse_epsiblock(Madreblocks{i},nb_channels);
    end
end
