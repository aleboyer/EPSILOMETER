function EPSI_concatanateSDMat(Meta_Data)
%
%  input: Meta_Data
...
if exist([ctdDIR 'SDctd_' Meta_Data.deployement '.mat'],'file')
    fprintf( 'load previsous data')
    load([ctdDIR 'SDctd_' Meta_Data.deployement '.mat'])
    load([epsiDIR 'SDepsi_' Meta_Data.deployement '.mat'])
else
    C=[];P=[];T=[];ctdtime=[];S=[];sig=[];
...
listepsi=dir([epsiDIR 'SDepsi_raw*.mat']);
listctd=dir([ctdDIR 'SDctd_raw*.mat']);
...
save([ctdDIR 'SDctd_' Meta_Data.deployement '.mat'],'ctdtime','C','T','P','S','sig','lastfile')
command=[];
for n=1:nbchannels
    command=[command ',' sprintf('''%s''',name_channels{n})];
end
command=['''index'',''epsitime'',''sderror'',''chsum1'',''alti'',''chsumepsi''' command];
command=sprintf('save(''%sSDepsi_%s.mat'',%s)',epsiDIR,Meta_Data.deployement,command);
eval(command);
